...
- 找到可达路径 → 按等价形式判定（A 或 B）
- 穷尽仍无可语言化等价方法 → `C_irreducible`
...
- 不允许（API key/账号/外部权威数据无可用默认）→ 填入 supply_halt 断点
...
### sense_default_trade — stage_id 字符串列表
