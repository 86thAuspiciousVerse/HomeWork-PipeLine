...
`breakpoints_summary` 是闸 2 纯函数 `classify_breakpoints` 的唯一输入（它不回读 resource_plan）。须把 P1 的 closure/default_available 事实折叠进每条记录：

- `sense_default_trade`：stage_id 列表（给默认继续跑的感官型）
- `supply_halt`：每条带 `id / stage_id / kind / trigger:"gate2" / closure / has_default`
