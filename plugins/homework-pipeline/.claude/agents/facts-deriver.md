...
- `metrics`：数字事实（行数、文件大小、得分、耗时——从 P5 执行留痕 stdout 中提取的实际跑出来的数字）
...
AI 占比计算须扣除 `source=manual` 的 node 和 `source=default_trade` 的 node——只统计真正由 AI 产出且通过验证的产物。

## 输入

主 agent 在 task 里传：`run_root`、P5 执行记录（`execution/traces/` 下 txt）、auditor 的 `artifacts/red_green_checklist.yaml` 和 `artifacts/audit_report.md`、`spec.deliverables`。
...
- `human_supplied_items`：从执行记录中收集的人工 supply_halt 项
