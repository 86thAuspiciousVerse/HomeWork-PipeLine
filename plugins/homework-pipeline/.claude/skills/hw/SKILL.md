...
插件根目录路径约定：`<plugin_root>` = `C:\Code\HomeWork-PipeLine\plugins\homework-pipeline`。以下所有 `python "<plugin_root>/.homework/orchestrator_state.py"` 均用此路径。

## 编码规范（Windows GBK 终端兼容）

所有由 LLM 生成的 Python 脚本（P5 阶段的 `code/<node>.py`）必须遵守：
1. **文件头加 `# -*- coding: utf-8 -*-`**。
2. **所有 `print()` 输出使用 ASCII-safe 符号**：
   - 用 `[PASS]` 代替 `✓`，用 `[FAIL]` 代替 `✗`，用 `[OK]` 代替 `√`
   - 所有 Unicode 特殊符号（`μg/m³`、`°C` 等）写入文件而非 print 到终端
3. **pandas 读 CSV 显式 `encoding="utf-8"`**（中文 Windows 上 Excel/GBK 默认编码会乱码）。
4. **执行 Python 脚本时设置 `PYTHONIOENCODING=utf-8`** 环境变量：
   ```bash
   set PYTHONIOENCODING=utf-8 && ".venv/Scripts/python.exe" "code/<node>.py"
   ```

## 启动
1. 用 Bash 跑 `markitdown "<docx>" -c UTF-8 -o <tmp.md> 2>&1` 把课程文档转成 Markdown，Read 它。
2. 用 Bash 调 `python "<plugin_root>/.homework/orchestrator_state.py" create-run "<docx 绝对路径>"`，拿回 `{run_id, run_root, state_path}`（state.yaml 已落盘，current_phase=SPEC_EXTRACT）。
3. **保存这三个值**：`run_id`、`run_root`（=`C:\Code\<cwd>\.homework\run_<id>`）、`state_path`。后续每个阶段委托 subagent 时必须注入 `run_root` 绝对路径到 task prompt。

## 委托 subagent 的统一模板

每次派 subagent 时，task 开头必须注入以下上下文（否则 agent 会把产物写到错误路径）：
...
代码目录: {run_root}/code/
所有产出文件（yaml/json/py）必须写入以上 run_root 目录树内，不得写入项目根目录。
...
每阶段 4 状态：`ENTERING → RUNNING → COMPLETED`；`PAUSED` 仅 supply_halt 触发。

**进入阶段的标准流程**：
1. `python "<plugin_root>/.homework/orchestrator_state.py" mark-entering <run_id> <PHASE>`（幂等，COMPLETED 不动）
2. 执行阶段任务（委托 subagent / Bash 调 CLI）
3. `python "<plugin_root>/.homework/orchestrator_state.py" commit-phase <run_id> <PHASE> "<artifact 相对 run_root 的路径>"`

`RUNNING` 不可信——重入时降回 ENTERING 幂等重跑。各阶段委托与产物：
...
- P4 PLAN_SELECTOR → Agent(plan-selector) → `artifacts/plan.yaml`（DAG + 每 node contract；被 REJECT 栈标 relaxed_verify，不改 spec）。**此阶段产物包含每 node 的 contract**——用于 P5 hw-exec 校验。
- P5 EXECUTOR → Agent(hw-orchestrator)，它建 `.homework/run_<id>/.venv`，然后对 DAG 每个 node：
  1. 从 plan.yaml 的 contract 生成 `execution/contract_<node>.yaml`（供 hw-exec 校验用）
  2. 用 Write 写 `code/<node>.py`（遵守上方编码规范）
  3. Bash 调 `set PYTHONIOENCODING=utf-8 && python "<plugin_root>/.homework/hw-exec" run-node --python <.venv>/Scripts/python --run-dir <run_root> --node <node>`（`--force-adopt-default` 旗用于 give_up 后有默认产物）
  4. hw-exec 取 node.status ∈ `pending|running|retriable|passed|passed_pending_b_judge|failed|given_up|supply_needed`（全 8 种），per node 冷启1+修码3
  5. 根据返回串 `/term2img` 截图，failed 喂 stdout 给 hw-orchestrator 改码重跑
